...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            if (PlayerJob ~= nil) and PlayerJob.name == "pdmowner" or PlayerJob.name == "pdmceo" or PlayerJob.name == "pdmmanager" or PlayerJob.name == "pdmemployee" or PlayerJob.name == "pdmlegal" then
                if IsPedInAnyVehicle(ped, false) then
                    if dist < 15 then
                        ...
                        if dist < 2 then
                            DrawText3Ds(bringcoords.x, bringcoords.y, bringcoords.z, '~g~E~w~ - Return the test vehicle')
                        ...
                        if not SellStarted then
                            ...
                            if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                                ...
                                QBCore.Functions.TriggerCallback('qb-vehicleshop:server:SellVehicle', function(SoldVehicle)
                                    if SoldVehicle then
                                        TriggerEvent("qb-phone:client:BankNotify", "$"..math.ceil(VehicleData["price"] / 100 * 60).." credited")
                                        local veh = GetVehiclePedIsIn(ped)
                                        QBCore.Functions.DeleteVehicle(veh)
                                    else
        ...
        if ClosestVehicle ~= nil then
            ...
            if not QB.ShowroomVehicles[ClosestVehicle].inUse then
                        if not vehshop.opened then
                            if PlayerJob.name == "pdmowner" or PlayerJob.name == "pdmceo" or PlayerJob.name == "pdmmanager" or PlayerJob.name == "pdmemployee" or PlayerJob.name == "pdmlegal" then
                                ...
                                DrawText3Ds(QB.ShowroomVehicles[ClosestVehicle].coords.x, QB.ShowroomVehicles[ClosestVehicle].coords.y, QB.ShowroomVehicles[ClosestVehicle].coords.z + 1.59, '~w~ Commission - ~g~'..com..'% (~g~$'..vehcommission..'~w~)')
                            else
